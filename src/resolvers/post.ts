...
export class PostResolver {
  /* format posts to only 50 characters */
  @FieldResolver(() => String)
  textSnippet(@Root() root: Post) {
    return root.text.slice(0, 50);
  }
  /* using dataloader to cache all users */
  @FieldResolver(() => User)
  creator(
    @Root() post: Post,
  ...
  async vote(
    ...
    // the user has voted in the post before
  ...
  async deletePost(@Arg("id", () => Int) id: number, @Ctx() { req }: MyContext): Promise<boolean> {
    // first delete updoot table not cascade way
